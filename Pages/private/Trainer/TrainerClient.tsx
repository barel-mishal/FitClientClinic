...
const TrainerClient: React.FC<Props> = ({ navigation, route: { params: {id}} }) => {
    ...
    const avgWorkoutsDuration = (workouts?.map(calcScore).reduce((a, b) => a + b, 0) ?? 0)  / (workouts?.length ?? 0);
    return <ScrollView style={{backgroundColor:"#f0f9ff"}}>
    <ClientView client={c} numberOfWorkout={workouts?.length ?? 0} avgWorkoutDuration={avgWorkoutsDuration} />
